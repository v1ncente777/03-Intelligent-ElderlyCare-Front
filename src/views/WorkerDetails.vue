...
export default {
  ...
  created() {
    this.$http.get( this.global.baseURL + this.global.workerURL.getOne + "/" + this.$route.params.id).then(result => {
      ...
      if (result.status === 200) {
        ...
        this.hire_date = result.data.data.Info.hire_date;
        this.resign_date = result.data.data.Info.resign_date;
  ...
  methods: {
    updateBlog() {
      this.$http
          .put(this.global.baseURL + this.global.workerURL.update + "/" + this.id, {
            id: this.id,
